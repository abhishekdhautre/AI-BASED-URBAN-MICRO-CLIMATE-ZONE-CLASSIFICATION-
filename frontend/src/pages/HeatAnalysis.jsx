...
const CHART_STYLE = {
  tooltip: { contentStyle: { backgroundColor: '#111827', border: '1px solid #1e2d45', borderRadius: '8px', fontSize: '12px' } },
  axis: { tick: { fill: '#64748b', fontSize: 11 }, axisLine: { stroke: '#1e2d45' }, tickLine: false },
}
...
export default function HeatAnalysis() {
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-1">Heat Analysis</h1>
        <p className="text-slate-400 text-sm">LCZ class relationships with Land Surface Temperature — Pune</p>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Coolest Zone', value: 'LCZ G (Water)', sub: '~24.1°C avg LST', color: 'text-blue-400' },
          { label: 'Heat Difference', value: '~19°C', sub: 'Paved vs Water', color: 'text-orange-400' },
          ...
          <BarChart data={HEAT_ANALYSIS_DATA} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
            ...
            <Bar dataKey="lst" radius={[3, 3, 0, 0]}>
              {HEAT_ANALYSIS_DATA.map((entry) => (
                <Cell key={entry.lcz} fill={entry.lst > 40 ? '#ef4444' : entry.lst > 35 ? '#f97316' : entry.lst > 30 ? '#eab308' : '#22c55e'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <ScatterChart margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" />
              <XAxis dataKey="vegetation" name="Vegetation %" {...CHART_STYLE.axis} label={{ value: 'Vegetation %', position: 'insideBottom', offset: -2, fill: '#64748b', fontSize: 11 }} />
        ...
        {/* Built-up vs Temperature */}
        <div className="card">
          <p className="section-title">Built-up Intensity vs LST</p>
          <ResponsiveContainer width="100%" height={240}>
            <ScatterChart margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e2d45" />
              <XAxis dataKey="builtup" name="Built-up %" {...CHART_STYLE.axis} label={{ value: 'Built-up %', position: 'insideBottom', offset: -2, fill: '#64748b', fontSize: 11 }} />
              <YAxis dataKey="lst" name="LST °C" {...CHART_STYLE.axis} />
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie data={HEAT_RISK_DATA} dataKey="count" nameKey="risk" cx="50%" cy="50%" outerRadius={90} label={({ risk, count }) => `${risk}: ${count}`} labelLine={false} fontSize={11}>
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            {[
              ...
              { color: '#3b82f6', title: 'Open layouts reduce heat retention', desc: 'Open LCZ types (4, 5, 6) are consistently cooler than compact equivalents at the same height.' },
